...
async function getownblogs(req, res){
    try{
        ...
        const [ results, fields ]  = await db.execute(
            "SELECT b.Blogid, b.Title, b.Content, b.Created, u.UserName, SUM(CASE WHEN r.Reaction = TRUE THEN 1 ELSE 0 END) AS PositiveReactions, SUM(CASE WHEN r.Reaction = FALSE THEN 1 ELSE 0 END) AS NegativeReactions FROM Blogs b LEFT JOIN Users u ON b.Userid = u.Userid LEFT JOIN Reactions r ON b.Blogid = r.Blogid WHERE u.Userid = ? GROUP BY b.Blogid, b.Title, b.Content, b.Created, u.UserName ORDER BY b.Created DESC LIMIT ? OFFSET ?",
            [userid, String(pageSize + 1), String(offset)]
        );
